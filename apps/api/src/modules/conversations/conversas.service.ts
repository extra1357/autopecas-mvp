...
export class ConversasService {
  ...
  async processarMensagem(telefone: string, mensagem: string): Promise<string> {
    ...
    if (contextoAtual.aguardandoNome && !cliente.nome) {
      ...
      const resposta = `Prazer, ${nome}! ??\n\nPosso te enviar promo��es, ofertas e novidades por WhatsApp? Responda *sim* ou *n�o*. Voc� pode cancelar quando quiser. ??`;
      await this.prisma.mensagem.create({
        data: { conversaId: conversa.id, origem: 'IA', conteudo: resposta },
      });
      return resposta;
    ...
    if (contextoAtual.aguardandoConsentimento && cliente.aceitaMarketing === null) {
      ...
      if (!aceitou && !recusou) {
        const resposta = `Por favor, responda *sim* ou *n�o*. Posso te enviar promo��es e novidades por WhatsApp?`;
        await this.prisma.mensagem.create({
          data: { conversaId: conversa.id, origem: 'IA', conteudo: resposta },
        });
        return resposta;
      ...
      const resposta = aceitou
        ? `�timo, ${nome}! Voc� receber� nossas melhores ofertas. ??\n\nAgora, qual pe�a voc� est� procurando? Me informe tamb�m o modelo e ano do ve�culo.`
        : `Sem problema, ${nome}! Voc� n�o receber� mensagens de marketing.\n\nQual pe�a voc� est� procurando? Me informe tamb�m o modelo e ano do ve�culo.`;
